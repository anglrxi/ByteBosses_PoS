...
public void addtable(int id, String Name, int Qty, Double Price) {
    ...
    private void initComponents() {
        ...
        jButton1.setIcon(new javax.swing.ImageIcon("C:\\Users\\QHTF\\Documents\\NetBeansProjects\\Simple_POS_Bytebosses\\src\\main\\java\\com\\mycompany\\pic\\1.png")); // NOI18N
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        addtable(1, "1PC CKN Meal", i, getPriceFromDB(1));
        cal();
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        addtable(2, "1PC CKN AC", i, getPriceFromDB(2));
        cal();
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        addtable(3, "Spaghetti Meal", i, getPriceFromDB(3));
        cal();
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        addtable(4, "Chick Pop AC", i, getPriceFromDB(4));
        cal();
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        ...
        addtable(5, "CB Meal", i, getPriceFromDB(5));
        cal();
    ...
    private void jButton6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton6ActionPerformed
        ...
        addtable(6, "Chick Pop Com", i, getPriceFromDB(6));
        cal();
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton7ActionPerformed
        ...
        addtable(7, "CKN Fil AC", i, getPriceFromDB(7));
        cal();
    ...
    private void jButton8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton8ActionPerformed
        ...
        addtable(8, "CKN Fil Meal", i, getPriceFromDB(8));
        cal();
    ...
    private void jButton9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton9ActionPerformed
        ...
        addtable(9, "CKN Fil Com", i, getPriceFromDB(9));
        cal();
    ...
    // Check if the table is empty before processing
    if (dt.getRowCount() == 0) {
        JOptionPane.showMessageDialog(this, "Please add items to the cart first!");
        return;
    }
    ...
    if (pay.getText().isEmpty()) {
        JOptionPane.showMessageDialog(this, "Please enter the cash amount!");
        return;
    }
    
    double paid = Double.valueOf(pay.getText());
    
    // Safety check: Don't process if payment is insufficient
    if (paid < tot) {
    ...
    try (Connection con = DatabaseConnection.getConnection()) {
        ...
        // We include user_id to satisfy the fk_sale_user constraint
        String saleSql = "INSERT INTO sales (user_id, total_amount) VALUES (?, ?)"; 
        PreparedStatement pstSale = con.prepareStatement(saleSql, java.sql.Statement.RETURN_GENERATED_KEYS);

        // Set the user_id from the session (must be 1 or 2 as per your DB dump)
        pstSale.setInt(1, UserSession.currentUserId); 
        pstSale.setDouble(2, tot);
        pstSale.executeUpdate();

        // Get the generated sale_id to link with sale_details
        java.sql.ResultSet rs = pstSale.getGeneratedKeys();
        int last_id = 0;
        if (rs.next()) { 
            last_id = rs.getInt(1); 
        }

        // STEP 2: Prepare Statements for the loop
        String detailSql = "INSERT INTO sale_details (sale_id, product_id, product_name, unit_price, quantity, total_price) VALUES (?, ?, ?, ?, ?, ?)";
        String updateStockSql = "UPDATE products SET qty_on_hand = qty_on_hand - ? WHERE product_name = ?";
        
        PreparedStatement pstDetail = con.prepareStatement(detailSql);
        PreparedStatement pstStock = con.prepareStatement(updateStockSql);

        // STEP 3: The Loop - Process each item in the JTable
        for (int i = 0; i < dt.getRowCount(); i++) {
        // Get the ID from column 0 (The first column in your JTable)
        int pid = Integer.parseInt(dt.getValueAt(i, 0).toString()); 
        String name = dt.getValueAt(i, 1).toString();
        int qnty = Integer.parseInt(dt.getValueAt(i, 2).toString());
        double price = Double.parseDouble(dt.getValueAt(i, 3).toString());
        double unit_p = price / qnty;

        // Set the parameters in the correct order
        pstDetail.setInt(1, last_id);    // sale_id
        pstDetail.setInt(2, pid);        // product_id (THIS WAS MISSING)
        pstDetail.setString(3, name);    // product_name
        pstDetail.setDouble(4, unit_p);  // unit_price
        pstDetail.setInt(5, qnty);       // quantity
        pstDetail.setDouble(6, price);   // total_price

        pstDetail.executeUpdate();

            // Update Product Stock[cite: 4]
            pstStock.setInt(1, qnty);
        ...
        // This will catch Foreign Key fails if UserSession.currentUserId is invalid[cite: 4]
    ...
    public double getPriceFromDB(int id) {
    double price = 0.0;
    try (Connection con = DatabaseConnection.getConnection()) {
